...
export function AnalyticsDashboard({ uid, profile, cs, lang="en", onBack, sessions:propSessions }) {
  ...
  function renderTabContent() {
    ...
    if (tab === "ai") return (<>
            ...
            {/* Executive Summary */}
            <Card title={`✦ ${t.executive}`}
              sub={isAr?"ملخص ذكاء اصطناعي لحالة وضعيتك":"AI-powered posture health executive summary"}
              action={
                <button onClick={()=>loadAI(true)} style={{ background:"none",
                  border:`1px solid ${T.border}`, borderRadius:R.xs,
                  padding:`${SP[1]}px ${SP[2]+2}px`, fontSize:10,
                  color:T.muted, cursor:"pointer" }}>↻ Refresh</button>
              }>
              {aiLoading.summary ? (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {[0,1,2].map(i=><Skeleton key={i} h={12} w={i===1?"70%":"100%"} r={4}/>)}
                  <div style={{ display:"flex", gap:8, alignItems:"center", fontSize:11, color:T.muted }}>
                    <Spinner size={14} color={T.blue}/>{t.loading}
                  </div>
                </div>
              ) : aiError.summary ? (
                <div style={{ fontSize:11, color:T.red }}>{aiError.summary}</div>
              ) : aiSummary ? (<>
                {/* Headline */}
                <div style={{ fontSize:15, fontWeight:700, color:T.text, lineHeight:1.5,
                  marginBottom:SP[3], borderLeft:`3px solid ${T.blue}`,
                  paddingLeft:SP[3] }}>
                  {aiSummary.headline}
                </div>
                {/* Risk gauges */}
                <div style={{ display:"flex", gap:SP[5], justifyContent:"center",
                  marginBottom:SP[4] }}>
                  <RiskGauge value={aiSummary.stats?.burnout_risk||0}
                    label={isAr?"خطر الإرهاق":"Burnout Risk"}/>
                  <RiskGauge value={aiSummary.stats?.posture_risk||0}
                    label={isAr?"خطر الوضعية":"Posture Risk"}/>
                  <div style={{ display:"flex", flexDirection:"column",
                    alignItems:"center", gap:6 }}>
                    <div style={{ fontSize:28, fontWeight:800,
                      color:aiSummary.stats?.trend_slope>0?T.green:aiSummary.stats?.trend_slope<0?T.red:T.muted }}>
                      {aiSummary.stats?.trend_label==="improving"?"↑":"improving"===aiSummary.stats?.trend_label?"↑":aiSummary.stats?.trend_label==="declining"?"↓":"→"}
                    </div>
                ...
                {/* Recommendations */}
                {aiSummary.top_recommendations?.length > 0 && (<>
                  <div style={{ fontSize:10, fontWeight:700, color:T.muted,
                    letterSpacing:".08em", textTransform:"uppercase", marginBottom:8 }}>
                    {isAr?"التوصيات الأهم":"Top Recommendations"}
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                    {aiSummary.top_recommendations.map((r,i)=>(
                ...
                <div style={{ fontSize:12, color:T.muted }}>
                  {isAr?"اضغط Refresh لتحليل بياناتك":"Click Refresh to analyze your data"}
                </div>
              )}
            </Card>
              ...
              {aiLoading.weekly ? (
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:SP[3] }}>
                  <div style={{ display:"flex", alignItems:"center",
                    ...
                    <Badge
                      label={(aiWeekly.performance||"fair").toUpperCase()}
                      color={aiWeekly.performance==="good"?T.green:aiWeekly.performance==="poor"?T.red:T.amber}
                      bg={aiWeekly.performance==="good"?C.greenDim:aiWeekly.performance==="poor"?"rgba(239,68,68,.08)":"rgba(245,158,11,.08)"}
                    />
                  </div>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:SP[2] }}>
                    <div style={{ background:"rgba(255,255,255,.025)", borderRadius:R.sm,
                      padding:SP[3], textAlign:"center" }}>
                      <div style={{ fontSize:24, fontWeight:800,
                        color:sc(aiWeekly.curr_avg||0) }}>{aiWeekly.curr_avg||"—"}</div>
                      <div style={{ fontSize:10, color:T.muted }}>{isAr?"هذا الأسبوع":"This Week"}</div>
                    </div>
                    <div style={{ background:"rgba(255,255,255,.025)", borderRadius:R.sm,
                      padding:SP[3], textAlign:"center" }}>
                      <div style={{ fontSize:24, fontWeight:800,
                        color:(aiWeekly.delta||0)>=0?T.green:T.red }}>
                        {(aiWeekly.delta||0)>=0?"+":""}{aiWeekly.delta||0}
                      </div>
                      <div style={{ fontSize:10, color:T.muted }}>{isAr?"التغيير":"Change"}</div>
                    </div>
                  </div>
                  {aiWeekly.top_insight && (
                    <div style={{ background:"rgba(99,102,241,.06)",
                      border:`1px solid rgba(99,102,241,.2)`, borderRadius:R.sm,
                      padding:`${SP[2]+1}px ${SP[3]}px`, fontSize:12, color:T.text,
                      lineHeight:1.6 }}>
                      💡 {aiWeekly.top_insight}
                    </div>
                  )}
                  {aiWeekly.action_this_week && (
                    <div style={{ fontSize:12, color:T.muted, lineHeight:1.6 }}>
                      <span style={{ fontWeight:700, color:T.blue }}>
                        {isAr?"إجراء هذا الأسبوع: ":"Action: "}
                      </span>
                      {aiWeekly.action_this_week}
                    </div>
                  )}
            ...
            <Card title={`🔮 ${t.predictive}`}
              sub={isAr?"توقع النقاط خلال 7 أيام":"7-day posture score forecast"}>
              {aiLoading.predict ? (
                <div style={{ display:"flex", gap:8, alignItems:"center", fontSize:11, color:T.muted }}>
                  <Spinner size={14} color={T.blue}/>{t.loading}
                </div>
                ...
                {/* Forecast bars */}
                {aiPredict.forecast?.length > 0 && (
                  <div style={{ marginBottom:SP[4] }}>
                    <div style={{ fontSize:11, fontWeight:700, color:T.muted,
                      marginBottom:SP[3] }}>
                      {isAr?"توقع النقاط (7 أيام)":"Score Forecast (7 days)"}
                    </div>
                    <div style={{ display:"flex", gap:5, height:80, alignItems:"flex-end" }}>
                      {aiPredict.forecast.map(f=>(
                        <ForecastBar key={f.day} {...f}/>
                      ))}
                    </div>
                  </div>
                )}
                <Divider/>
                {/* Anomalies */}
                {aiPredict.anomaly_count > 0 && (
                  <div style={{ marginBottom:SP[3] }}>
                    <div style={{ fontSize:11, fontWeight:700, color:T.amber,
                      marginBottom:6, display:"flex", alignItems:"center", gap:6 }}>
                      ⚠️ {aiPredict.anomaly_count} {isAr?"شذوذ مكتشف":"anomalies detected"}
                    </div>
                    {aiPredict.anomalies?.slice(0,3).map((a,i)=>(
                      <div key={i} style={{ fontSize:11, color:T.muted, marginBottom:3 }}>
                        • {a.date}: score {a.score}/100 ({a.delta} from avg)
                      </div>
                    ))}
                  </div>
                )}
                {/* Narrative */}
                {aiPredict.narrative?.burnout_prediction && (
                  <div style={{ fontSize:12, color:T.text, lineHeight:1.7 }}>
                    {aiPredict.narrative.burnout_prediction}
                  </div>
                )}
                {aiPredict.narrative?.intervention_urgency === "immediate" && (
                  <div style={{ marginTop:SP[3], padding:`${SP[2]+1}px ${SP[3]}px`,
                    background:"rgba(239,68,68,.08)", border:"1px solid rgba(239,68,68,.2)",
                    borderRadius:R.sm, fontSize:12, color:T.red, fontWeight:600 }}>
                    ⚡ {isAr?"تدخل فوري مطلوب":"Immediate intervention recommended"}
                  </div>
                )}
              </>) : (
                <div style={{ fontSize:11, color:T.muted }}>
            ...
            {/* Fatigue Analysis */}
            <Card title={`⏰ ${t.fatigue}`}
              sub={isAr?"متى تكون وضعيتك أسوأ في اليوم":"When during the day your posture degrades"}>
              {aiLoading.fatigue ? (
                <div style={{ display:"flex", gap:8, alignItems:"center", fontSize:11, color:T.muted }}>
                  <Spinner size={14} color={T.blue}/>{t.loading}
                </div>
              ) : aiFatigue ? (<>
                <HourlyHeatmap hourly={aiFatigue.hourly||{}}/>
                <div style={{ marginTop:SP[3], display:"grid",
                  gridTemplateColumns:"1fr 1fr 1fr", gap:SP[2] }}>
                  {[
                    [isAr?"الصباح":"Morning",   aiFatigue.morning_avg],
                    [isAr?"الظهيرة":"Afternoon", aiFatigue.afternoon_avg],
                    [isAr?"المساء":"Evening",    aiFatigue.evening_avg],
                  ].map(([label,val])=>(
                    <div key={label} style={{ background:"rgba(255,255,255,.025)",
                      borderRadius:R.sm, padding:SP[3], textAlign:"center" }}>
                      <div style={{ fontSize:18, fontWeight:800,
                        color:val?sc(val):T.muted }}>{val||"—"}</div>
                      <div style={{ fontSize:9, color:T.muted, marginTop:2 }}>{label}</div>
                    </div>
                  ))}
                </div>
                {aiFatigue.peak_fatigue_description && (
                  <div style={{ marginTop:SP[3], fontSize:12, color:T.text, lineHeight:1.6 }}>
                    {aiFatigue.peak_fatigue_description}
                  </div>
                )}
                {aiFatigue.break_schedule?.length > 0 && (
                  <div style={{ marginTop:SP[3] }}>
                    <div style={{ fontSize:10, fontWeight:700, color:T.muted,
                      letterSpacing:".08em", textTransform:"uppercase", marginBottom:6 }}>
                      {isAr?"جدول الاستراحات المقترح":"Recommended Break Schedule"}
                    </div>
                    {aiFatigue.break_schedule.map((b,i)=>(
                      <div key={i} style={{ fontSize:11, color:T.text,
                        marginBottom:3 }}>• {b}</div>
                    ))}
                  </div>
                )}
              </>) : null}
...
function KpiBox({ label, value, sub, color, icon, trend, accent }) {
  ...
  const loadAI = useCallback(async (force=false) => {
    ...
    const sessData = sessions.map(s => ({
      ...
      good_pct: s.good_pct||0,
      mode: s.mode||"laptop",
      created_at_ms: s.created_at?.toDate?.()?.getTime() || 0,
    }));
    const prof = { name:profile?.name||"", tier:profile?.tier||"standard" };

    // Executive summary
    if (!aiSummary || force) {
      setAiLoading(p=>({...p,summary:true}));
      AIAPI.executiveSummary({ sessions:sessData, profile:prof, lang })
        .then(d => setAiSummary(d))
        .catch(e => setAiError(p=>({...p,summary:e.message})))
        .finally(()=>setAiLoading(p=>({...p,summary:false})));
    }
    // Predictive
    if (!aiPredict || force) {
      setAiLoading(p=>({...p,predict:true}));
      AIAPI.predictive({ sessions:sessData, lang })
        .then(d => setAiPredict(d))
        .catch(e => setAiError(p=>({...p,predict:e.message})))
        .finally(()=>setAiLoading(p=>({...p,predict:false})));
    }
    // Weekly
    if (!aiWeekly || force) {
      setAiLoading(p=>({...p,weekly:true}));
      AIAPI.weeklyInsights({ sessions:sessData, lang })
        .then(d => setAiWeekly(d))
        .catch(e => setAiError(p=>({...p,weekly:e.message})))
        .finally(()=>setAiLoading(p=>({...p,weekly:false})));
    }
    // Fatigue
    if (!aiFatigue || force) {
      setAiLoading(p=>({...p,fatigue:true}));
      AIAPI.fatigueAnalysis({ sessions:sessData, lang })
        .then(d => setAiFatigue(d))
        .catch(e => setAiError(p=>({...p,fatigue:e.message})))
        .finally(()=>setAiLoading(p=>({...p,fatigue:false})));
    }
  }, [sessions, profile, lang, aiSummary, aiPredict, aiWeekly, aiFatigue]);

  useEffect(() => {
    if (tab === "ai" && sessions.length > 0) loadAI();
  }, [tab, sessions.length]); // eslint-disable-line
  ...
  const TABS = [
    ...
    { id:"ai",       label:t.ai,       icon:"✦", badge:"AI" },
