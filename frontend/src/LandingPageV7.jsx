...
import { ONLINE_PAYMENT_LIVE, whatsappActivationLink, activateLabel, activationPromise } from "./lib/salesWhatsapp.js";
              ...
              <div className={p.popular ? "lp-lift lp-glow lp-popular-card" : "lp-lift"} style={{
                ...
                {p.isEnterprise ? (
                  ...
                  <>
                    ...
                    {/* WhatsApp manual-activation — shown only when online checkout is off and plan has a price */}
                    {!ONLINE_PAYMENT_LIVE && p.priceEGP && (p.priceEGP.monthly > 0 || p.priceEGP.yearly > 0) && (
                      <a
                        href={whatsappActivationLink({
                          planName: ar ? (p.nameAr || p.name) : p.name,
                          billing,
                          price: billing === "yearly" ? Math.round((p.priceEGP.yearly||0)/12) : (p.priceEGP.monthly||0),
                          currency: "EGP",
                          isAr: ar,
                        })}
                      ...
                      >
                        ...
                        {activateLabel(ar)} · {ar ? (p.nameAr || p.name) : p.name}
                      </a>
                    )}
                  </>
                )}
