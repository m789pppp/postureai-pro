  ...
  const toolGroups = isHR ? [
    {
      ...
      items: [
        ...
        { id:"t-reports",   icon:"📋", en:"Team Reports",   ar:"تقارير الفريق",
          onClick:()=>{ uid&&getUserSessions(uid).then(setUserSessions).catch(()=>{}); setShowAIReports?.(true); }},
        { id:"t-insights",  icon:"🧠", en:"AI Insights",    ar:"رؤى AI",
          onClick:()=>{ uid&&getUserSessions(uid).then(setUserSessions).catch(()=>{}); setShowAIInsights?.(true); }},
        { id:"t-predict",   icon:"🔮", en:"Burnout AI",     ar:"AI إرهاق",
          onClick:()=>{ uid&&getUserSessions(uid).then(setUserSessions).catch(()=>{}); setShowPredictiveAI?.(true); }},
      ],
    },
    {
      id: "enterprise",
