/**
 * PostureAI Pro — AI Reports Engine v1.0
...
<h2>${isAr ? "🧠 التحليل الذكي — Claude AI" : "🧠 AI Analysis — Claude AI"}</h2>
<div class="ai-box">
  <div class="ai-label">🧠 ${isAr ? "PostureAI Intelligence" : "PostureAI Intelligence"}</div>
...
<div class="footer">
  <span>PostureAI Pro — ${isAr ? "تقرير سري" : "Confidential Report"}</span>
  <span>${isAr ? "أُنشئ بواسطة Claude AI" : "Generated by Claude AI"} · ${now}</span>
</div>
</body>
</html>`;
}
...
export function AIReports({ profile, sessions = [], allUsers = [], cs, lang = "en", onClose }) {
  ...
  const system = `You are PostureAI's report generation engine. Generate professional, concise reports in ${isAr ? "Arabic" : "English"}.
...
## Recognition & Improvement Plan`,
  ...
  const loadReport = useCallback(async (key) => {
    ...
    try {
      const text = await callClaude(prompts[key]?.() || "", system, 1200);
  ...
  const exportPDF = async () => {
    ...
    if (!summary) {
      try { summary = await callClaude(prompts.summary(), system, 800); }
