...
// Kept to 2 models, not 3+: this is an Edge Function, whose real
// platform execution limit is NOT controlled by vercel.json's
// functions.maxDuration (that only applies to Node.js serverless
// functions). Each model here gets its own short timeout independent
// of the outer budget, so one slow model can't eat the time meant for
// the next model — or for pollText/pollChat, which already ran before
// this and used up part of the shared 22s ceiling.
async function llm7(msgs, maxTok, temp, signal) {
  for (const model of ["gpt-4o-mini", "meta-llama/llama-3.3-70b-instruct"]) {
    ...
    try {
      ...
      if (signal?.aborted && e.name === "AbortError") throw e; // whole-request timeout — stop entirely
      // otherwise just this model's own per-call timeout — try the next one
    } finally {
